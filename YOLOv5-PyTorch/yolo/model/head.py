...
class Head(nn.Module):
    def __init__(self, predictor, anchors, strides,
                 match_thresh, eiou_ratio, loss_weights,
    ...
    def compute_loss(self, preds, targets):
        ...
        for pred, stride, wh in zip(preds, self.strides, self.anchors):
            ...
            if len(anchor_id) > 0:
                ...
                eiou = box_ops.box_eiou(box_grid, gt_boxes[gt_id] / stride).to(dtype)
                losses["loss_box"] += (1 - eiou).mean()

                gt_object[image_id, grid_xy[:, 1], grid_xy[:, 0], anchor_id] = \
                    self.eiou_ratio * eiou.detach().clamp(0) + (1 - self.eiou_ratio)
